...
Jikes RVM (Research Virtual Machine) dostarcza społeczności badaczy
elastyczne, otwarte środowisko testowe do eksperymentowania z
technologiami maszyn wirtualnych i dużym zakresem alternatyw
projektowych.

Jikes RVM działa na platformach Linux/IA-32, AIX/PowerPC, OSX/PowerPC
oraz Linux/PowerPC i rozszerza technologie maszyn wirtualnych o
dynamiczną kompilację, optymalizację adaptacyjną, odśmiecanie,
szeregowanie wątków oraz synchronizację. Cechą charakterystyczną Jikes
RVM jest to, że jest zaimplementowany w języku Java i samonośny, tzn.
jego kod w Javie działa na nim samym bez wymagania drugiej maszyny
wirtualnej. Większość innych maszyn wirtualnych dla platformy Javy
jest napisana w kodzie natywnym (zwykle C lub C++). Implementacja w
Javie ułatwia przenośność i przezroczystą integrację zasobów maszyny
wirtualnej i aplikacji, takich jak obiekty, wątki i interfejsy
